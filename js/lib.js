...
Admin.lib = {};
Admin.lib.getLocaleMes = function(_name,_locale){
	if(!_locale){
...
Admin.lib.getEntityNameByClassName = function(_class){
...
Admin.lib.getObjectByPriority = function(){
...
Admin.lib.prepareControllerName = function(_className){
	return  'Controller' + _className;
};

Admin.lib.isEmpty = function(_ob){
	for(var v in _ob){
		return false;
	}
	return true;
...
Admin.lib.isEmptyString = function(_str){
...
Admin.lib.calcEvalForObject = function(_ob, _evalString){
	var str = _evalString;
	try{
		str = eval(str);
	}catch(e){}
...
Admin.lib.copyObject = function(_ob){
	if(_ob == null){
		return null;
	}
	var copy = null;
	...
	case 'object' : if(Object.prototype.toString.call(_ob) == "[object Array]"){
						copy = new Array();
						for(var i in _ob){
							copy[i] = Admin.lib.copyObject(_ob[i]);
						}
						return copy;
					}else{
						copy = {};
...
Admin.lib.objectToString = function(_ob){
	if(_ob == null){
		return 'NULL';
	}
	switch(typeof _ob){
	...
	case 'object' : var res = '';
					if(Object.prototype.toString.call(_ob) == "[object Array]"){
						...
						for(var i in _ob){
							...
							res += Admin.lib.objectToString(_ob[i]);
						}
						res += ']';
					}else{
						res += '={';
						...
						for(var i in _ob){
							...
							res += i + ':' + Admin.lib.objectToString(_ob[i]);
						}
						res += '}';
					}
					return res;
...
Admin.lib.prepareDivIdForVtObject = function(_windowLevel,_entityClass, _fieldName){
	return 'div_' + _windowLevel + '_' + _entityClass + '_' + _fieldName;
};


Admin.lib.echoString = function(_str){
...
if(Admin.lib.model == undefined)Admin.lib.model = {};

Admin.lib.model.defaultPagedLoading = true;
Admin.lib.model.defaultIsComposition = false;
Admin.lib.model.defaultIsIdentifier = false;
Admin.lib.model.defaultIsNullable = true;
Admin.lib.model.defaultIsDetached = false;
Admin.lib.model.defaultMaxSize = 100000;
Admin.lib.model.defaultMinSize = 0;
Admin.lib.model.defaultMax = 99999999999999999999;
Admin.lib.model.defaultMin = -99999999999999999999;


/*
 * определяет, является ли данная сущность загружаемой постранично или нет
 */
Admin.lib.model.pagedLoading = function(_entityClass){
 ...
 */
Admin.lib.model.isComposition = function(_entityClass, _fieldName){
	if(model.CLASSES[_entityClass] == undefined){
		log.error('Не задана модель для класса ' + _entityClass + ' (model.CLASSES.' + _entityClass + ')');
 ...
 */
Admin.lib.model.isIdentifier = function(_entityClass, _fieldName){
	if(model.CLASSES[_entityClass] == undefined){
		log.error('Не задана модель для класса ' + _entityClass + ' (model.CLASSES.' + _entityClass + ')');
 ...
 * _fieldName - название поля, для которого определяется значение параметра
 */
Admin.lib.model.isNullable = function(_entityClass, _fieldName){
	if(model.CLASSES[_entityClass] == undefined){
		log.error('Не задана модель для класса ' + _entityClass + ' (model.CLASSES.' + _entityClass + ')');
 ...
 * _fieldName - название поля, для которого определяется значение параметра
 */
Admin.lib.model.isDetached = function(_entityClass, _fieldName){
	if(model.CLASSES[_entityClass] == undefined){
		log.error('Не задана модель для класса ' + _entityClass + ' (model.CLASSES.' + _entityClass + ')');
 ...
 * _fieldName - название поля, для которого определяется значение параметра
 */
Admin.lib.model.maxSize = function(_entityClass, _fieldName){
	if(model.CLASSES[_entityClass] == undefined){
		log.error('Не задана модель для класса ' + _entityClass + ' (model.CLASSES.' + _entityClass + ')');
 ...
 * _fieldName - название поля, для которого определяется значение параметра
 */
Admin.lib.model.minSize = function(_entityClass, _fieldName){
	if(model.CLASSES[_entityClass] == undefined){
		log.error('Не задана модель для класса ' + _entityClass + ' (model.CLASSES.' + _entityClass + ')');
 ...
 * _fieldName - название поля, для которого определяется значение параметра
 */
Admin.lib.model.max = function(_entityClass, _fieldName){
	if(model.CLASSES[_entityClass] == undefined){
		log.error('Не задана модель для класса ' + _entityClass + ' (model.CLASSES.' + _entityClass + ')');
 ...
 * _fieldName - название поля, для которого определяется значение параметра
 */
Admin.lib.model.min = function(_entityClass, _fieldName){
	if(model.CLASSES[_entityClass] == undefined){
		log.error('Не задана модель для класса ' + _entityClass + ' (model.CLASSES.' + _entityClass + ')');
...
log.debug = function(_str,_objs){
	...
	if(_objs != undefined){
		console.debug(_str + ': ' + Admin.lib.objectToString(_objs));
	}else{
		console.debug(_str);
	}
};
...
Admin.lib.getMulticast = function(_ob){
	if(!Admin.lib.isEmpty(_ob.mediaSources)){
